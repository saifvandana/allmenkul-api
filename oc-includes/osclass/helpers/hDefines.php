...
function osc_item_deactivate_url($secret = '', $id = '') {
  if ($id == '') { $id = osc_item_id(); }
  if ( osc_rewrite_enabled() ) {
    return osc_base_url() . osc_get_preference('rewrite_item_deactivate') . '/' . $id . '/' . $secret;
  } else {
...
function osc_get_parent_domain() {
  $domain = parse_url(osc_base_url(), PHP_URL_HOST);
  $domain = explode('.', $domain);
  $domain = array_slice($domain, -2, 2);
  $domain = implode('.', array_filter($domain));
  return $domain;
}
