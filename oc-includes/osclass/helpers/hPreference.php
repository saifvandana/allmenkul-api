...
function osc_location_url($code = '', $pattern = '', $sort = '') {
  ...
  if($code == '' && $pattern == '') {
  return $api_url . '?action=list&apiVersion=' . $api_version . '&sort=' . $sort;
  } else if($code <> '') {
  return $api_url . '?action=find&apiVersion=' . $api_version . '&code=' . $code;
  } else if($pattern <> '') {
  return $api_url . '?action=search&apiVersion=' . $api_version . '&pattern=' . $pattern . '&sort=' . $sort;
  }
}
