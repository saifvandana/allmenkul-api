...
if(@$_GET['ajaxQuery'] == '1' && @$_GET['pattern'] <> '') {
  $max = 8;
...
if(@$_GET['ajaxItem'] == '1' && @$_GET['sPattern'] <> '') {
  ...
  foreach( $prepare as $i => $p ) {
    ...
    $prepare[$i]['i_price'] = alp_ajax_item_format_price($prepare[$i]['i_price'], $prepare[$i]['fk_c_currency_code']);
...
if(@$_GET['ajaxPhoneClick'] == '1' && @$_GET['itemId'] > 0) {
  alp_increase_clicks($_GET['itemId'], $_GET['itemUserId']);
  echo 1;
  exit;
}
