...
<?php 
  if($type == 'itemviewer') {
    ...
    require_once osc_base_path().'oc-content/themes/alpha/item.php'; 
    exit;
  }
?>  
...
<body id="body-item-forms" class="fw-supporting">
  ...
  <?php if($type == 'friend') { ?>
    ...
    <div id="send-friend-form" class="fw-box" style="display:block;">
      <div class="head">
        <h1><?php _e('Send to friend', 'alpha'); ?></h1>
      </div>
      ...
      <div class="middle">
        ...
        <form target="_top" id="sendfriend" name="sendfriend" action="<?php echo osc_base_url(true); ?>" method="post">
          <fieldset>
            ...
            <?php } else { ?>
              <div class="row">
                <label for="yourName"><span><?php _e('Your name', 'alpha'); ?></span><span class="req">*</span></label> 
                <div class="input-box"><?php SendFriendForm::your_name(); ?></div>

                <label for="yourEmail"><span><?php _e('Your e-mail address', 'alpha'); ?></span><span class="req">*</span></label>
                <div class="input-box"><?php SendFriendForm::your_email(); ?></div>
              </div>
            <?php } ?>

            <div class="row">
              <label for="friendName"><span><?php _e("Your friend's name", 'alpha'); ?></span><span class="req">*</span></label>
              <div class="input-box"><?php SendFriendForm::friend_name(); ?></div>

              <label for="friendEmail"><span><?php _e("Your friend's e-mail address", 'alpha'); ?></span><span class="req">*</span></label>
              <div class="input-box last"><?php SendFriendForm::friend_email(); ?></div>
            </div>
                  
            <div class="row last">
              <label for="message"><span><?php _e('Message', 'alpha'); ?></span><span class="req">*</span></label>
              <?php SendFriendForm::your_message(); ?>
            </div>

            <?php alp_show_recaptcha(); ?>

            <button type="<?php echo (alp_param('forms_ajax') == 1 ? 'button' : 'submit'); ?>" id="send-message" class="alpBg item-form-submit" data-type="friend"><?php _e('Send message', 'alpha'); ?></button>
          </fieldset>
        </form>
  ...
  <?php if($type == 'comment') { ?>
    ...
    <?php if( osc_comments_enabled() && (osc_reg_user_post_comments () && osc_is_web_user_logged_in() || !osc_reg_user_post_comments()) ) { ?>
      <form target="_top" action="<?php echo osc_base_url(true) ; ?>" method="post" name="comment_form" id="comment_form" class="fw-box" style="display:block;">
        ...
        <fieldset>
          <div class="head">
            <h1><?php _e('Add a new comment', 'alpha'); ?></h1>
          </div>
          ...
          <div class="middle">
            ...
            <?php } else { ?>
              <div class="row">
                <label for="authorName"><?php _e('Name', 'alpha') ; ?></label> 
                <div class="input-box"><?php CommentForm::author_input_text(); ?></div>
              </div>

              <div class="row">
                <label for="authorEmail"><span><?php _e('E-mail', 'alpha') ; ?></span><span class="req">*</span></label> 
                <div class="input-box"><?php CommentForm::email_input_text(); ?></div>
              </div>                  
            <?php } ?>

            <div class="row" id="last">
              <label for="title"><?php _e('Title', 'alpha') ; ?></label>
              <div class="input-box"><?php CommentForm::title_input_text(); ?></div>
            </div>
        
            <div class="row">
              <label for="body"><span><?php _e('Message', 'alpha'); ?></span><span class="req">*</span></label>
              <?php CommentForm::body_input_textarea(); ?>
            </div>

            <?php alp_show_recaptcha(); ?>

            <button type="<?php echo (alp_param('forms_ajax') == 1 ? 'button' : 'submit'); ?>" id="send-comment" class="alpBg item-form-submit" data-type="comment"><?php _e('Submit comment', 'alpha') ; ?></button>
          </div>
        </fieldset>
      </form>
    <?php } ?>
  ...
  <?php if($type == 'contact') { ?>
    ...
    <form target="_top" action="<?php echo osc_base_url(true) ; ?>" method="post" name="contact_form" id="contact_form" class="fw-box" style="display:block;">
      ...
      <fieldset>
        <div class="head">
          <h1><?php _e('Contact seller', 'alpha'); ?></h1>
        </div>
        ...
        <div class="middle">
          ...
          <?php if( osc_item_is_expired () ) { ?>
            <div class="problem">
              <?php _e('This listing expired, you cannot contact seller.', 'alpha') ; ?>
            </div>
          <?php } else if( (osc_logged_user_id() == osc_item_user_id()) && osc_logged_user_id() != 0 ) { ?>
            <div class="problem">
              <?php _e('It is your own listing, you cannot contact yourself.', 'alpha') ; ?>
            </div>
          <?php } else if( osc_reg_user_can_contact() && !osc_is_web_user_logged_in() ) { ?>
            <div class="problem">
              <?php _e('You must log in or register a new account in order to contact the advertiser.', 'alpha') ; ?>
            </div>
          <?php } else { ?> 


            <div class="row">
              <label for="yourName"><?php _e('Name', 'alpha') ; ?><span class="req">*</span></label> 
              <div class="input-box"><?php ContactForm::your_name(); ?></div>
            </div>

            <div class="row">
              <label for="yourEmail"><span><?php _e('E-mail', 'alpha') ; ?></span><span class="req">*</span></label> 
              <div class="input-box"><?php ContactForm::your_email(); ?></div>
            </div>              

            <div class="row">
              <label for="phoneNumber"><span><?php _e('Phone', 'alpha') ; ?></span></label> 
            ...
            <button type="<?php echo (alp_param('forms_ajax') == 1 ? 'button' : 'submit'); ?>" id="send-message" class="alpBg item-form-submit" data-type="contact"><?php _e('Send message', 'alpha') ; ?></button>
          <?php } ?>
        </div>
      </fieldset>
    </form>
        ...
        <div class="middle">
          <fieldset>
            ...
            <?php if($user_id == osc_logged_user_id() && osc_is_web_user_logged_in()) { ?>
              <div class="problem"><?php _e('This is your own profile!', 'alpha'); ?></div>
            <?php } else { ?>
              <?php if(!osc_is_web_user_logged_in()) { ?>
                <div class="row">
                  <label for="yourName"><?php _e('Name', 'alpha'); ?></label> 
                  <div class="input-box"><?php ContactForm::your_name(); ?></div>
                </div>
              ...
              <div class="row last">
                <label for="phoneNumber"><span><?php _e('Phone number', 'alpha') ; ?></span></label>
                <div class="input-box"><?php ContactForm::your_phone_number(); ?></div>
              </div>
              ...
              <button type="<?php echo (alp_param('forms_ajax') == 1 ? 'button' : 'submit'); ?>" id="send-public-message" class="alpBg item-form-submit" data-type="contact_public"><?php _e('Send message', 'alpha') ; ?></button>
            <?php } ?>
          </fieldset>
        </div>
      </form>
