<?php
define('ALPHA_THEME_VERSION', '1.5.3');

require_once osc_base_path() . 'oc-content/themes/alpha/model/ModelALP.php';


function alp_theme_info() {
  return array(
    'name' => 'Alpha Osclass Theme',
    'version' => '1.5.3',
    'description' => 'Fast and eye-catching osclass premium theme',
    'author_name' => 'MB Themes',
    'author_url' => 'https://osclasspoint.com',
    'support_uri' => 'https://forums.osclasspoint.com/alpha-osclass-theme/',
...
function alp_public_profile_items() {
  $section = osc_get_osclass_section();  
  if(osc_get_osclass_location() == 'user' && ($section == 'items' || $section == 'pub_profile')) {
    Params::setParam('itemsPerPage', alp_param('public_items'));
  }
}

osc_add_hook('init', 'alp_public_profile_items');


// CHECK IF LAZY LOAD ENABLED
function alp_is_lazy() {
  if(alp_param('lazy_load') == 1 && osc_get_preference('force_aspect_image', 'osclass') == 0) {
    return true;
  }
...
// RELATED ADS
function alp_related_ads() {
  if(alp_param('related') == 1) {
    $limit = (alp_param('related_count') > 0 ? alp_param('related_count') : 3);
    ...
    if(osc_count_items() > 0) {
      ...
      echo '<h3>' . __('You may also be interested in', 'alpha') . '</h3>';
...
function alp_locale_post_links() {
  ...
  if(count($locales) > 0) {
    ...
    foreach($locales as $l) {
      $html .= '<a href="#" data-locale="' . $l['pk_c_code'] . '" data-name="' . $l['s_name'] . '" class="alpBgActive' . ($c == $l['pk_c_code'] ? ' active' : '') . '">' . $l['s_short_name'] . '</a>';
    }
...
function alp_profile_picture($user_id = null, $size = 'small') {
  if($user_id === null) {
...
function alp_search_param_remove() {
  ...
  foreach($params as $n => $v) {
    if(!in_array($n, array('page')) && ($v > 0 || $v <> '')) {
      $output[$n] = array(
        'value' => $v, 
        'param' => $n,
        'title' => alp_param_name($n),
        'name' => alp_remove_value_name($v, $n)
      );
    }
  }
...
function alp_remove_value_name($value, $type) {
  $def_cur = (alp_param('def_cur') <> '' ? alp_param('def_cur') : '$');

  if($type == 'sPeriod') {  
    return alp_get_simple_name($value, 'period');

  } else if($type == 'sTransaction') {  
    return alp_get_simple_name($value, 'transaction');

  } else if($type == 'sCondition') {  
    return alp_get_simple_name($value, 'condition');
    ...
    return ($value == 1 ? __('Yes', 'alpha') : __('No', 'alpha'));
...
function alp_list_options($type) {
  $opt = array();
  ...
  if($type == 'condition') {
    ...
    $opt[2] = __('Buy', 'alpha');
    $opt[3] = __('Rent', 'alpha');
    $opt[4] = __('Exchange', 'alpha');

  } else if ($type == 'period') {
    $opt[0] = __('All', 'alpha');
    $opt[1] = __('Yesterday', 'alpha');
    $opt[7] = __('Last week', 'alpha');
    $opt[14] = __('Last 2 weeks', 'alpha');
    $opt[31] = __('Last month', 'alpha');
    $opt[365] = __('Last year', 'alpha');

  } else if ($type == 'seller_type') {
    $opt[0] = __('All', 'alpha');
    $opt[1] = __('Personal', 'alpha');
    $opt[2] = __('Company', 'alpha');
  }
...
function alp_get_simple_name($id, $type) {
  $options = alp_list_options($type);
  return @$options[$id];
}


// DEFAULT LOCATION PICKER CONTENT
function alp_def_location() {
  $html = '';

  $type = (alp_param('def_locations') == '' ? 'region' : alp_param('def_locations'));
  ...
  $type_name = ($type == 'region' ? __('region', 'alpha') : __('city', 'alpha'));


  foreach($countries as $c) {
    $html .= '<div class="option country init" data-country="' . $c['pk_c_code'] . '" data-region="" data-city="" data-code="country' . $c['pk_c_code'] . '" id="' . $c['pk_c_code'] . '"><strong>' . osc_esc_js(osc_location_native_name_selector($c, 's_name')) . '</strong></div>';

    if($type == 'city') {
      $regions_cities = ModelAlp::newInstance()->getCities($c['pk_c_code'], $limit, $city_not_empty);
    }
    ...
    foreach($regions_cities as $r) {
      if($counter < $limit) {
        if(strtoupper($r['fk_c_country_code']) == strtoupper($c['pk_c_code'])) {
          if($type == 'region') {
            $html .= '<div class="option region init" data-country="' . $r['fk_c_country_code'] . '" data-region="' . $r['pk_i_id'] . '" data-city="" data-code="region' . $r['pk_i_id'] . '" id="' . $r['pk_i_id'] . '"><strong>' . osc_esc_js(osc_location_native_name_selector($r, 's_name')) . '</strong><span>' . osc_esc_js(osc_location_native_name_selector($c, 's_name')) . '</span></div>';
          } else { 
            $html .= '<div class="option region init" data-country="' . $r['fk_c_country_code'] . '" data-region="' . $r['fk_i_region_id'] . '" data-city="' . $r['pk_i_id'] . '" data-code="city' . $r['pk_i_id'] . '" id="' . $r['pk_i_id'] . '"><strong>' . osc_esc_js(osc_location_native_name_selector($r, 's_name')) . '</strong><span>' . osc_esc_js(osc_location_native_name_selector($c, 's_region_name')) . '</span></div>';
          }
        }
      }
    ...
    if($counter == $limit) {
      $html .= '<div class="option service empty-pick default" data-country="" data-region="" data-city="" data-code="" id=""><em>' . osc_esc_js(sprintf(__('... and %d more %s, enter your %s name to refine results', 'alpha'), $limit, $type_name, $type_name)) . '</em></div>';
    }
  }
...
function alp_fancy_url($type, $params = array()) {
  ...
  return alp_item_send_friend_url() . $url . $extra;
}


// CUSTOM SEND FRIEND URL
function alp_item_send_friend_url($item_id = '') {
  if($item_id <= 0) {
...
// GET THEME PARAM
function alp_param($name) {
  return osc_get_preference($name, 'theme-alpha');
}


// CHECK IF PRICE ENABLED ON CATEGORY
function alp_check_category_price($id) {
...
function alp_is_rtl() {
  ...
  if(in_array($current_lang, alp_rtl_languages())) {
    return true;
  } else {
    return false;
  }
...
function alp_flat_categories() {
  return '<div id="flat-cat-fancy" style="display:none;overflow:hidden;">' . alp_category_loop() . '</div>';
}


// SMART DATE
function alp_smart_date( $time ) {
  ...
  if($time_diff < 2) {
    $time_diff_name = __('minute ago', 'alpha');
  } else if ($time_diff < 60) {
    $time_diff_name = sprintf(__('%d minutes ago', 'alpha'), $time_diff);
  } else if ($time_diff < 120) {
    $time_diff_name = sprintf(__('%d hour ago', 'alpha'), $time_diff_h);
  } else if ($time_diff < 1440) {
    $time_diff_name = sprintf(__('%d hours ago', 'alpha'), $time_diff_h);
  } else if ($time_diff < 2880) {
    $time_diff_name = sprintf(__('%d day ago', 'alpha'), $time_diff_d);
  } else if ($time_diff < 10080) {
    $time_diff_name = sprintf(__('%d days ago', 'alpha'), $time_diff_d);
  } else if ($time_diff < 20160) {
    $time_diff_name = sprintf(__('%d week ago', 'alpha'), $time_diff_w);
  } else if ($time_diff < 43200) {
    $time_diff_name = sprintf(__('%d weeks ago', 'alpha'), $time_diff_w);
  } else if ($time_diff < 86400) {
    $time_diff_name = sprintf(__('%d month ago', 'alpha'), $time_diff_m);
  } else if ($time_diff < 518400) {
    $time_diff_name = sprintf(__('%d months ago', 'alpha'), $time_diff_m);
  } else if ($time_diff < 1036800) {
    $time_diff_name = sprintf(__('%d year ago', 'alpha'), $time_diff_y);
  } else {
    $time_diff_name = sprintf(__('%d years ago', 'alpha'), $time_diff_y);
  }
...
function alp_check_sold(){
  ...
  if($status <> '' && $id <> '' && $id > 0) {
    ...
    if( $secret == $item['s_secret'] ) {
      //Item::newInstance()->dao->update(DB_TABLE_PREFIX.'t_item_alpha', array('i_sold' => $status), array('fk_i_item_id' => $item['pk_i_id']));
      $comm->update(DB_TABLE_PREFIX.'t_item_alpha', array('i_sold' => $status), array('fk_i_item_id' => $item['pk_i_id']));
...
osc_add_hook('header', 'alp_check_sold');



// HELP FUNCTION TO GET CATEGORIES
function alp_category_loop( $parent_id = NULL, $parent_color = NULL ) {
  $parent_color = isset($parent_color) ? $parent_color : NULL;

  if(Params::getParam('sCategory') <> '') {
    $id = Params::getParam('sCategory');
  } else if (alp_get_session('sCategory') <> '' && (osc_is_publish_page() || osc_is_edit_page())) {
    $id = alp_get_session('sCategory');
  ...
  $html .= '<div class="single info">' . __('Select category', 'alpha') . ' ' . ($parent_id <> 0 ? '<span class="back tr1 round2"><i class="fa fa-angle-left"></i> ' . __('Back', 'alpha') . '</span>' : '') . '</div>';

  foreach( $categories as $c ) {
    if( $parent_id == 0) {
      $parent_color = alp_get_cat_color( $c['pk_i_id'] );
      $icon = '<div class="parent-icon" style="background:' . alp_get_cat_color( $c['pk_i_id'] ) . ';">' . alp_get_cat_icon( $c['pk_i_id'] ) . '</div>';
    } else {
      $icon = '<div class="parent-icon children" style="background: ' . $parent_color . '">' . alp_get_cat_icon( $c['pk_i_id'] ) . '</div>';
    }
    ...
    if(isset($subcategories[0])) {
      $html .= alp_category_loop( $c['pk_i_id'], $parent_color );
    }
  }
...
function alp_flat_category_select(){  
  $root = Category::newInstance()->findRootCategoriesEnabled();

  $html = '<div class="category-box tr1">';
  foreach( $root as $c ) {
    $html .= '<div class="option tr1" style="background:' . alp_get_cat_color( $c['pk_i_id'] ) . ';">' . alp_get_cat_icon( $c['pk_i_id'] ) . '</div>';
  }
...
function alp_user_menu() {
  ...
  $options = array();
  $options[] = array('name' => __('My listings', 'alpha'), 'url' => osc_user_list_items_url(), 'class' => 'opt_items', 'icon' => 'fa-check-square-o', 'section' => 1, 'count' => 0);
  //$options[] = array('name' => __('Active', 'alpha'), 'url' => osc_user_list_items_url() . $s_active, 'class' => 'opt_active_items', 'icon' => 'fa-check-square-o', 'section' => 1, 'count' => $c_active, 'is_active' => $yes_active);
  //$options[] = array('name' => __('Not Validated', 'alpha'), 'url' => osc_user_list_items_url() . $s_pending, 'class' => 'opt_not_validated_items', 'icon' => 'fa-stack-overflow', 'section' => 1, 'count' => $c_pending, 'is_active' => $yes_pending);
  //$options[] = array('name' => __('Expired', 'alpha'), 'url' => osc_user_list_items_url() . $s_expired, 'class' => 'opt_expired_items', 'icon' => 'fa-times-circle', 'section' => 1, 'count' => $c_expired, 'is_active' => $yes_expired);
  //$options[] = array('name' => __('Dashboard', 'alpha'), 'url' => osc_user_dashboard_url(), 'class' => 'opt_dashboard', 'icon' => 'fa-dashboard', 'section' => 2);
  $options[] = array('name' => __('Alerts', 'alpha'), 'url' => osc_user_alerts_url(), 'class' => 'opt_alerts', 'icon' => 'fa-bullhorn', 'section' => 2);
  $options[] = array('name' => __('My profile', 'alpha'), 'url' => osc_user_profile_url(), 'class' => 'opt_account', 'icon' => 'fa-file-text-o', 'section' => 2);
  $options[] = array('name' => __('Public profile', 'alpha'), 'url' => osc_user_public_profile_url(), 'class' => 'opt_publicprofile', 'icon' => 'fa-picture-o', 'section' => 2);
  $options[] = array('name' => __('Logout', 'alpha'), 'url' => osc_user_logout_url(), 'class' => 'opt_logout', 'icon' => 'fa-sign-out', 'section' => 3);
  ...
  echo '<div class="img">';
  echo '<div class="box"><img src="' . alp_profile_picture(osc_logged_user_id(), 'large') . '"/></div><strong>' . osc_logged_user_name() . '</strong>';

  if(function_exists('profile_picture_show')) { 
    echo '<a href="#" class="update-avatar"><i class="fa fa-upload"></i>' . __('Update', 'alpha') . '</a>';
  }

  echo '</div>';

  echo '<div class="um s1">';
  echo '<div class="user-menu-header">' . __('My listings', 'alpha') . '</div>';
  ...
  echo '<div class="user-menu-header">' . __('My account', 'alpha') . '</div>';
  echo '<ul class="user_menu">';
...
function alp_get_term($term = '', $country = '', $region = '', $city = ''){
  if( $term == '') {
...
function alp_get_full_loc($country = '', $region = '', $city = ''){
  if( $city <> '' && is_numeric($city) ) {
    $city_info = City::newInstance()->findByPrimaryKey( $city );
...
function alp_extra_add_admin( $catId = null, $item_id = null ){
  ...
  if (strpos($current_url, $admin_url) !== false) {
    if($item_id > 0) {
      ...
      $item_extra = alp_item_extra( $item_id );

      echo '<div class="control-group">';
      echo '<label class="control-label" for="sTransaction">' . __('Transaction', 'alpha') . '</label>';
      echo '<div class="controls">' . alp_simple_transaction(true, $item_id <> '' ? $item_id : false) . '</div>';
      echo '</div>';

      echo '<div class="control-group">';
      echo '<label class="control-label" for="sCondition">' . __('Condition', 'alpha') . '</label>';
      echo '<div class="controls">' . alp_simple_condition(true, $item_id <> '' ? $item_id : false) . '</div>';
      echo '</div>';

      echo '<div class="control-group">';
      echo '<label class="control-label" for="sPhone">' . __('Phone', 'alpha') . '</label>';
      echo '<div class="controls"><input type="text" name="sPhone" id="sPhone" value="' . $item_extra['s_phone'] . '" /></div>';
      echo '</div>';

      echo '<div class="control-group">';
      echo '<label class="control-label" for="sSold">' . __('Item Sold', 'alpha') . '</label>';
...
osc_add_hook('item_form', 'alp_extra_add_admin');
osc_add_hook('item_edit', 'alp_extra_add_admin');



function alp_extra_edit( $item ) {
...
function alp_simple_category($select = false, $level = 3, $id = 'sCategory') {
  ...
  if(!$select) {
    ...
    $html .= '<span class="text round3 tr1"><span>' . (@$c_category['s_name'] <> '' ? $c_category['s_name'] : __('Category', 'alpha')) . '</span> <i class="fa fa-angle-down"></i></span>';
    $html .= '<div class="list">';
    $html .= '<div class="option info">' . __('Choose one category', 'alpha') . '</div>';

    if($id <> 'catId') {
      $html .= '<div class="option bold' . ($root['pk_i_id'] == "" ? ' selected' : '') . '" data-id="">' . __('All', 'alpha') . '</div>';
    }
    ...
    $html .= '<option value="" ' . ($root['pk_i_id'] == "" ? ' selected="selected"' : '') . '>' . __('All categories', 'alpha') . '</option>';
...
function alp_simple_seller( $select = false ) {
  ...
  $name = alp_get_simple_name($id_mod, 'seller_type');
  $name = ($name == '' ? __('Seller type', 'alpha') : $name);
  ...
  if( !$select ) {
    ...
    $html .= '<div class="list">';
    $html .= '<div class="option info">' . __('Choose seller type', 'alpha') . '</div>';
    $html .= '<div class="option bold' . ($id_mod == 0 ? ' selected' : '') . '" data-id="">' . __('All', 'alpha') . '</div>';

    $html .= '<div class="option' . ($id_mod == "1" ? ' selected' : '') . '" data-id="0">' . __('Personal', 'alpha') . '</span></div>';
    $html .= '<div class="option' . ($id_mod == "2" ? ' selected' : '') . '" data-id="1">' . __('Company', 'alpha') . '</span></div>';
    ...
    $html .= '<option value="" ' . ($id_mod == "0" ? ' selected="selected"' : '') . '>' . __('All sellers', 'alpha') . '</option>';
    $html .= '<option value="0" ' . ($id_mod == "1" ? ' selected="selected"' : '') . '>' . __('Personal', 'alpha') . '</option>';
    $html .= '<option value="1" ' . ($id_mod == "2" ? ' selected="selected"' : '') . '>' . __('Company', 'alpha') . '</option>';
    $html .= '</select>';
...
function alp_simple_transaction( $select = false, $item_id = false ) {
  if((osc_is_publish_page() || osc_is_edit_page()) && alp_get_session('sTransaction') <> '') {
    $id = alp_get_session('sTransaction');
  } else {
  ...
  if( $item_id > 0 ) {
    $id = alp_item_extra( $item_id );
    $id = $id['i_transaction'];
  }

  $name = alp_get_simple_name($id, 'transaction');
  $name = ($name == '' ? __('Transaction', 'alpha') : $name);

  $options =  alp_list_options('transaction');
  ...
  if( !$select ) {
    ...
    $html .= '<span class="text round3 tr1"><span>' . $name . '</span> <i class="fa fa-angle-down"></i></span>';
    $html .= '<div class="list">';
    $html .= '<div class="option info">' . __('Choose transaction type', 'alpha') . '</div>';
...
function alp_simple_condition( $select = false, $item_id = false ) {
  if((osc_is_publish_page() || osc_is_edit_page()) && alp_get_session('sCondition') <> '') {
    $id = alp_get_session('sCondition');
  } else {
  ...
  $name = alp_get_simple_name($id, 'condition');
  $name = ($name == '' ? __('Condition', 'alpha') : $name);

  $options =  alp_list_options('condition');
  ...
  if( !$select ) {
    ...
    $html .= '<span class="text round3 tr1"><span>' . $name . '</span> <i class="fa fa-angle-down"></i></span>';
    $html .= '<div class="list">';
    $html .= '<div class="option info">' . __('Choose condition of item', 'alpha') . '</div>';
...
function alp_simple_currency() {
  $currencies = osc_get_currencies();
  $item = osc_item(); 

  if((osc_is_publish_page() || osc_is_edit_page()) && alp_get_session('currency') <> '') {
    $id = alp_get_session('currency');
  } else {
  ...
  $html .= '<div class="list">';
  $html .= '<div class="option info">' . __('Currency', 'alpha') . '</div>';
...
function alp_simple_price_type() {
  $item = osc_item(); 
  ...
  if( isset($item['i_price']) ) {
    if( $item['i_price'] > 0 ) {
      $default_key = 0;
      $default_name = '<i class="fa fa-pencil help"></i> ' . __('Enter price', 'alpha');
    } else if( $item['i_price'] == 0 ) {
      $default_key = 1;
      $default_name = '<i class="fa fa-cut help"></i> ' . __('Free', 'alpha');
    } else if( $item['i_price'] == '' ) {
      $default_key = 2;
      $default_name = '<i class="fa fa-phone help"></i> ' . __('Check with seller', 'alpha');
    } 
  ...
  $html .= '<div class="list">';
  $html .= '<div class="option info">' . __('Choose price type', 'alpha') . '</div>';

  $html .= '<div class="option' . ($default_key == 0 ? ' selected' : '') . '" data-id="0"><i class="fa fa-pencil help"></i> ' . __('Enter price', 'alpha') . '</span></div>';
  $html .= '<div class="option' . ($default_key == 1 ? ' selected' : '') . '" data-id="1"><i class="fa fa-cut help"></i> ' . __('Free', 'alpha') . '</span></div>';
  $html .= '<div class="option' . ($default_key == 2 ? ' selected' : '') . '" data-id="2"><i class="fa fa-phone help"></i> ' . __('Check with seller', 'alpha') . '</span></div>';
...
function alp_simple_period( $select = false ) {
  $id = Params::getParam('sPeriod');

  $name = alp_get_simple_name($id, 'period');
  $name = ($name == '' ? __('Age', 'alpha') : $name);

  $options =  alp_list_options('period');
  ...
  if( !$select ) {
    ...
    $html .= '<span class="text round3 tr1"><span>' . $name . '</span> <i class="fa fa-angle-down"></i></span>';
    $html .= '<div class="list">';
    $html .= '<div class="option info">' . __('Choose period', 'alpha') . '</div>';
...
function alp_category_root( $category_id ) {
...
// CREATE ITEM (in loop)
function alp_draw_item($c = NULL, $premium = false, $class = false) {
  if($premium){
...
function alp_random_items($numItems = 10, $category = array(), $withPicture = false) {
  ...
  $withPicture = alp_param('latest_picture');
  $randomOrder = alp_param('latest_random');
  $premiums = alp_param('latest_premium');
  $category = alp_param('latest_category');
...
function alp_item_location($premium = false) {
  if(!$premium) {
    $loc = array_filter(array(osc_item_city(), osc_item_region(), osc_item_country_code()));
  } else {
    $loc = array_filter(array(osc_premium_city(), osc_premium_region(), osc_premium_country_code()));
  }
...
function alp_location_format($country = null, $region = null, $city = null) { 
  if($country <> '') {
    if(strlen($country) == 2) {
    ...
    if($region <> '') {
      if($city <> '') {
        return $city . ' ' . __('in', 'alpha') . ' ' . $region . (osc_location_native_name_selector($country_full, 's_name') <> '' ? ' (' . osc_location_native_name_selector($country_full, 's_name') . ')' : '');
      } else {
        return $region . ' (' . osc_location_native_name_selector($country_full, 's_name') . ')';
      }
    } else { 
      if($city <> '') {
        return $city . ' ' . __('in', 'alpha') . ' ' . osc_location_native_name_selector($country_full, 's_name');
      } else {
    ...
    if($region <> '') {
      if($city <> '') {
        return $city . ' ' . __('in', 'alpha') . ' ' . $region;
      } else {
        return $region;
      }
    } else { 
      if($city <> '') {
        ...
        return __('Location not entered', 'alpha');
      }
    }
  }
}
...
function mb_filter_extend() {
  ...
  Search::newInstance()->addJoinTable( DB_TABLE_PREFIX.'t_item_alpha.fk_i_item_id', DB_TABLE_PREFIX.'t_item_alpha', DB_TABLE_PREFIX.'t_item.pk_i_id = '.DB_TABLE_PREFIX.'t_item_alpha.fk_i_item_id', 'LEFT OUTER' ) ; // Mod


  // SEARCH - TRANSACTION
  if(Params::getParam('sTransaction') > 0) {
    Search::newInstance()->addConditions(sprintf("%st_item_alpha.i_transaction = %d", DB_TABLE_PREFIX, Params::getParam('sTransaction')));
  }


  // SEARCH - CONDITION
  if(Params::getParam('sCondition') > 0) {
    Search::newInstance()->addConditions(sprintf("%st_item_alpha.i_condition = %d", DB_TABLE_PREFIX, Params::getParam('sCondition')));
  }
...
// GET ALL PARAMS
function alp_search_params_all() {
  $params = Params::getParamsAsArray();
  unset($params['iPage']);
  return $params;
}
...
// FIND MAXIMUM PRICE
function alp_max_price($cat_id = null, $country_code = null, $region_id = null, $city_id = null) {
...
function alp_premium_formated_price($price = null) {
  if($price == '') {
    $price = osc_premium_price();
  }

  return (string) alp_premium_format_price($price);
}

function alp_premium_format_price($price, $symbol = null) {
  if ($price === null) return osc_apply_filter ('item_price_null', __('Check with seller', 'alpha') );
  if ($price == 0) return osc_apply_filter ('item_price_zero', __('Free', 'alpha') );
...
function alp_ajax_item_format_price($price, $symbol_code) {
  if ($price === null) return __('Check with seller', 'alpha');
  if ($price == 0) return __('Free', 'alpha');
  return round($price/1000000, 2) . ' ' . $symbol_code;
}



AdminMenu::newInstance()->add_menu(__('Theme Setting', 'alpha'), osc_admin_render_theme_url('oc-content/themes/alpha/admin/header.php'), 'alpha_menu');
AdminMenu::newInstance()->add_submenu_divider('alpha_menu', __('Theme Settings', 'alpha'), 'alpha_submenu');
AdminMenu::newInstance()->add_submenu('alpha_menu', __('Configure', 'alpha'), osc_admin_render_theme_url('oc-content/themes/alpha/admin/configure.php'), 'settings_alpha1');
AdminMenu::newInstance()->add_submenu('alpha_menu', __('Advertisement', 'alpha'), osc_admin_render_theme_url('oc-content/themes/alpha/admin/banner.php'), 'settings_alpha2');
AdminMenu::newInstance()->add_submenu('alpha_menu', __('Category Icons', 'alpha'), osc_admin_render_theme_url('oc-content/themes/alpha/admin/category.php'), 'settings_alpha3');
AdminMenu::newInstance()->add_submenu('alpha_menu', __('Logo', 'alpha'), osc_admin_render_theme_url('oc-content/themes/alpha/admin/header.php'), 'settings_alpha4');
AdminMenu::newInstance()->add_submenu('alpha_menu', __('Plugins', 'alpha'), osc_admin_render_theme_url('oc-content/themes/alpha/admin/plugins.php'), 'settings_alpha5');
...
if( !function_exists('logo_header') ) {
  function logo_header() {
    ...
    if(file_exists(WebThemes::newInstance()->getCurrentThemePath() . 'images/logo.jpg')) {
      return $html;
    } else if( alp_param('default_logo') && (file_exists( WebThemes::newInstance()->getCurrentThemePath() . 'images/default-logo.jpg')) ) {
...
if(!function_exists('alp_theme_install')) {
  $themeInfo = alp_theme_info();

  function alp_theme_install() {
    osc_set_preference('version', ALPHA_THEME_VERSION, 'theme-alpha');
    osc_set_preference('color', '#f29c12', 'theme-alpha');
    osc_set_preference('site_info', __('Widely known as Worlds no. 1 online classifieds platform, our is all about you. Our aim is to empower every person in the country to independently connect with buyers and sellers online.', 'alpha'), 'theme-alpha');
    osc_set_preference('date_format', 'mm/dd', 'theme-alpha');
    osc_set_preference('cat_icons', '0', 'theme-alpha');
    osc_set_preference('footer_link', '1', 'theme-alpha');
    osc_set_preference('default_logo', '1', 'theme-alpha');
    osc_set_preference('def_cur', '$', 'theme-alpha');
    osc_set_preference('def_view', '0', 'theme-alpha');
    osc_set_preference('website_name', 'AlphaTheme', 'theme-alpha');
    osc_set_preference('latest_picture', '0', 'theme-alpha');
    osc_set_preference('latest_random', '1', 'theme-alpha');
    osc_set_preference('latest_premium', '0', 'theme-alpha');
    osc_set_preference('latest_category', '', 'theme-alpha');
    osc_set_preference('publish_category', '1', 'theme-alpha');
    osc_set_preference('premium_home', '1', 'theme-alpha');
    osc_set_preference('premium_search', '0', 'theme-alpha');
    osc_set_preference('premium_home_count', '12', 'theme-alpha');
    osc_set_preference('premium_search_count', '3', 'theme-alpha');
    osc_set_preference('search_ajax', '1', 'theme-alpha');
    osc_set_preference('forms_ajax', '1', 'theme-alpha');
    osc_set_preference('post_required', '', 'theme-alpha');
    osc_set_preference('post_extra_exclude', '', 'theme-alpha');
    osc_set_preference('home_layout', 't', 'theme-alpha');
    osc_set_preference('favorite_home', '1', 'theme-alpha');
    osc_set_preference('blog_home', '1', 'theme-alpha');
    osc_set_preference('company_home', '1', 'theme-alpha');
    osc_set_preference('banners', '0', 'theme-alpha');
    osc_set_preference('lazy_load', '0', 'theme-alpha');
    osc_set_preference('public_items', '24', 'theme-alpha');
    osc_set_preference('preview', '0', 'theme-alpha');
    osc_set_preference('related', '1', 'theme-alpha');
    osc_set_preference('related_count', '6', 'theme-alpha');
    osc_set_preference('def_locations', 'region', 'theme-alpha');


    /* Banners */
    if(function_exists('alpha_banner_list')) {
      foreach(alp_banner_list() as $b) {
        osc_set_preference($b['id'], '', 'theme-alpha');
      }
    }

    osc_reset_preferences();

    alp_add_item_fields();  // add extra item fiels into database
  }
}


if(!function_exists('check_install_alp_theme')) {
  function check_install_alp_theme() {
    $current_version = alp_param('version');
...
check_install_alp_theme();


// WHEN NEW LISTING IS CREATED, ADD IT TO ALPHA EXTRA TABLE
function alp_new_item_extra($item) {
  $conn = DBConnectionClass::newInstance();
  $data = $conn->getOsclassDb();
  $comm = new DBCommandClass($data);
  $db_prefix = DB_TABLE_PREFIX;

  $query = "INSERT INTO {$db_prefix}t_item_alpha (fk_i_item_id) VALUES ({$item['pk_i_id']})";
  $result = $comm->query($query);
}

osc_add_hook('posted_item', 'alp_new_item_extra');


// WHEN NEW CATEGORY IS CREATED, ADD IT TO ALPHA EXTRA TABLE
function alp_new_category_extra() {
  ...
  $query = "INSERT INTO {$db_prefix}t_category_alpha (fk_i_category_id) 
            SELECT c.pk_i_id FROM {$db_prefix}t_category c WHERE c.pk_i_id NOT IN (SELECT d.fk_i_category_id FROM {$db_prefix}t_category_alpha d)";
  $result = $comm->query($query);
}

osc_add_hook('footer', 'alp_new_category_extra');



// USER MENU FIX
function alp_user_menu_fix() {
  $user = User::newInstance()->findByPrimaryKey(osc_logged_user_id());
  View::newInstance()->_exportVariableToView('user', $user);
}

osc_add_hook('header', 'alp_user_menu_fix');



// ADD THEME COLUMNS INTO ITEM TABLE
function alp_add_item_fields() {
  ModelALP::newInstance()->install();
}



// UPDATE THEME COLS ON ITEM POST-EDIT
function alp_update_fields($item) {
  ...
  Item::newInstance()->dao->update(DB_TABLE_PREFIX.'t_item_alpha', $fields, array('fk_i_item_id' => $item['pk_i_id']));
}

osc_add_hook('posted_item', 'alp_update_fields');
osc_add_hook('edited_item', 'alp_update_fields');


// GET ALPHA ITEM EXTRA VALUES
function alp_item_extra($item_id) {
  if($item_id > 0) {
    $db_prefix = DB_TABLE_PREFIX;
    $query = "SELECT * FROM {$db_prefix}t_item_alpha s WHERE fk_i_item_id = " . $item_id . ";";
...
// GET ALPHA CATEGORY EXTRA VALUES
function alp_category_extra($category_id) {
  if($category_id > 0) {
    $db_prefix = DB_TABLE_PREFIX;

    $query = "SELECT * FROM {$db_prefix}t_category_alpha s WHERE fk_i_category_id = " . $category_id . ";";
...
osc_add_hook('pre_item_post', 'alp_post_preserve');


// DROP VALUES OF INPUTS ON SUCCESSFUL PUBLISH
function alp_post_drop() {
...
osc_add_hook('posted_item', 'alp_post_drop');



// GET VALUES FROM SESSION ON PUBLISH PAGE
function alp_get_session( $param ) {
...
function alp_default_colors() {
  $colors = array(1 => '#F44336', 2 => '#00BCD4', 3 => '#009688', 4 => '#FDE74C', 5 => '#8BC34A', 6 => '#D32F2F', 7 => '#2196F3', 8 => '#777', 999 => '#F44336');
  return $colors;
}


function alp_get_cat_icon( $id, $string = false ) {
  $category = Category::newInstance()->findByPrimaryKey( $id );
  $category_extra = alp_category_extra($id);
  $default_icons = alp_default_icons();

  if(alp_param('cat_icons') == 1) { 
    ...
    if($string) {
      ...
      if(file_exists(osc_base_path() . 'oc-content/themes/alpha/images/small_cat/' . $category['pk_i_id'] . '.png')) {
...
function alp_get_cat_color( $id ) {
  $category = Category::newInstance()->findByPrimaryKey( $id );
  $category_extra = alp_category_extra($id);
  $default_colors = alp_default_colors();
...
function alp_increase_clicks($itemId, $itemUserId = NULL) {
  if($itemId > 0) {
    if($itemUserId == '' || $itemUserId == 0 || ($itemUserId <> '' && $itemUserId > 0 && $itemUserId <> osc_logged_user_id())) {
      $db_prefix = DB_TABLE_PREFIX;
      $query = 'INSERT INTO ' . $db_prefix . 't_item_stats_alpha (fk_i_item_id, dt_date, i_num_phone_clicks) VALUES (' . $itemId . ', "' . date('Y-m-d') . '", 1) ON DUPLICATE KEY UPDATE  i_num_phone_clicks = i_num_phone_clicks + 1';
...
function alp_admin_menu_fix(){
  echo '<style>' . PHP_EOL;
  echo 'body.compact #alpha_menu .ico-alpha_menu {bottom:-6px!important;width:50px!important;height:50px!important;margin:0!important;background:#fff url(' . osc_base_url() . 'oc-content/themes/alpha/images/favicons/favicon-32x32.png) no-repeat center center !important;}' . PHP_EOL;
  echo 'body.compact #alpha_menu .ico-alpha_menu:hover {background-color:rgba(255,255,255,0.95)!important;}' . PHP_EOL;
  echo 'body.compact #menu_alpha_menu > h3 {bottom:0!important;}' . PHP_EOL;
  echo 'body.compact #menu_alpha_menu > ul {border-top-left-radius:0px!important;margin-top:1px!important;}' . PHP_EOL;
  echo 'body.compact #menu_alpha_menu.current:after {content:"";display:block;width:6px;height:6px;border-radius:10px;box-shadow:1px 1px 3px rgba(0,0,0,0.1);position:absolute;left:3px;bottom:3px;background:#03a9f4}' . PHP_EOL;
  echo 'body:not(.compact) #alpha_menu .ico-alpha_menu {background:transparent url(' . osc_base_url() . 'oc-content/themes/alpha/images/favicons/favicon-32x32.png) no-repeat center center !important;}' . PHP_EOL;
  echo '</style>' . PHP_EOL;
}

osc_add_hook('admin_header', 'alp_admin_menu_fix');
...
function alp_phone_clicks( $item_id ) {
  if( $item_id <> '' ) {
    $db_prefix = DB_TABLE_PREFIX;

    $query = "SELECT sum(coalesce(i_num_phone_clicks, 0)) as phone_clicks FROM {$db_prefix}t_item_stats_alpha s WHERE fk_i_item_id = " . $item_id . ";";
...
function alp_show_recaptcha( $section = '' ){
...
// SHOW BANNER
function alp_banner( $location ) {
  $html = '';

  if(alp_param('banners') == 1) {
    if( alp_is_demo() ) {
      $class = ' is-demo';
    } else {
      $class = '';
    ...
    if(alp_param('banner_' . $location) == '') {
      $blank = ' blank';
    } else {
      $blank = '';
    }

    if( alp_is_demo() && alp_param('banner_' . $location) == '' ) {
      $title = ' title="' . __('You can define your own banner code from theme settings', 'alpha') . '"';
    } else {
      $title = '';
    }

    $html .= '<div class="banner-theme banner-' . $location . ' not767' . $class . $blank . '"' . $title . '><div>';
    $html .= alp_param('banner_' . $location);

    if( alp_is_demo() && alp_param('banner_' . $location) == '' ) {
      $html .= __('Banner space', 'alpha') . ': <u>' . $location . '</u>';
    }
...
function alp_banner_list() {
  $list = array(
    array('id' => 'banner_home_top', 'position' => __('Top of home page', 'alpha')),
    array('id' => 'banner_home_bottom', 'position' => __('Bottom of home page', 'alpha')),
    array('id' => 'banner_search_sidebar', 'position' => __('Bottom of search sidebar', 'alpha')),
    array('id' => 'banner_search_top', 'position' => __('Top of search page', 'alpha')),
    array('id' => 'banner_search_bottom', 'position' => __('Bottom of search page', 'alpha')),
    array('id' => 'banner_search_list', 'position' => __('On third position between search listings (list view)', 'alpha')),
    array('id' => 'banner_item_top', 'position' => __('Top of item page', 'alpha')),
    array('id' => 'banner_item_bottom', 'position' => __('Bottom of item page', 'alpha')),
    array('id' => 'banner_item_sidebar', 'position' => __('Bottom of item sidebar', 'alpha')),
    array('id' => 'banner_item_description', 'position' => __('Under item description', 'alpha'))
  );
...
function alp_extra_fields_hide() {
  $list = trim(alp_param('post_extra_exclude'));
  $array = explode(',', $list);
...
osc_add_hook('header', 'alp_disable_404');


// THEME PARAMS UPDATE
if(!function_exists('alp_param_update')) {
  function alp_param_update( $param_name, $update_param_name, $type = NULL, $plugin_var_name = null ) {
...
function alp_cat_tree() {
  ...
  foreach($root as $c) {
    ...
    $array[$i]['sub'] = alp_cat_sub($c['pk_i_id']);
    $i++;
  }
...
function alp_cat_list($selected = array(), $categories = '', $level = 0) {
  if($categories == '') {
    $categories = alp_cat_tree();
  }
  ...
  foreach($categories as $c) {
    ...
    if(isset($c['sub']) && count($c['sub']) > 0) {
      alp_cat_list($selected, $c['sub'], $level + 1);
    }
  }
}
