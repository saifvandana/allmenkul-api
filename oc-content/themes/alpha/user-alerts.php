  ...
  <div class="inside user_account">
    ...
    <div id="main" class="alerts">
      ...
      <div class="inside">
        <?php if(osc_count_alerts() > 0) { ?>
          ...
          <?php while(osc_has_alerts()) { ?>
            ...
            <div class="alert" >
              <div class="top">
                <span class="menu">
                  <strong><?php echo __('Alert', 'alpha') . ' #' . $c; ?></strong>
                  <span><?php echo osc_count_items(); ?> <?php echo (osc_count_items() == 1 ? __('item found', 'alpha') : __('items found', 'alpha')); ?></span>
                </span>
                <a class="btn alpBg" onclick="javascript:return confirm('<?php echo osc_esc_js(__('This action can\'t be undone. Are you sure you want to continue?', 'alpha')); ?>');" href="<?php echo osc_user_unsubscribe_alert_url(); ?>"><span><?php _e('Unsubscribe', 'alpha'); ?></span></a>
              </div>

              <div class="param">
                <div class="elem w33 <?php if($alert_details['sPattern'] == '') { ?>no-entry<?php } ?>">
                  <div class="left"><?php _e('Pattern', 'alpha'); ?></div>
                ...
                <div class="elem w33 <?php if($alert_details['price_min'] == 0) { ?>no-entry<?php } ?>">
                  <div class="left"><?php _e('Min. price', 'alpha'); ?></div>
                  <div class="right"><?php if($alert_details['price_min'] == 0) { echo '--'; } else { echo $alert_details['price_min'] . alp_param('def_cur'); } ?></div>
                </div>

                <div class="elem w33 <?php if($alert_details['price_max'] == 0) { ?>no-entry<?php } ?>">
                  <div class="left"><?php _e('Max. price', 'alpha'); ?></div>
                  <div class="right"><?php if($alert_details['price_max'] == 0) { echo '--'; } else { echo $alert_details['price_max'] . alp_param('def_cur'); } ?></div>
                </div>

                <div class="elem w33 <?php if($alert_details['countries'] == '' or empty($c_prepare)) { ?>no-entry<?php } ?>">
                  <div class="left"><?php _e('Country', 'alpha'); ?></div>
                  <div class="right">
                    <?php 
                ...
                <div class="elem w33 <?php if($alert_details['regions'] == '' or empty($r_prepare)) { ?>no-entry<?php } ?>">
                  <div class="left"><?php _e('Region', 'alpha'); ?></div>
                  <div class="right">
                    <?php 
                ...
                <div class="elem w33 <?php if($alert_details['cities'] == '' or empty($t_prepare)) { ?>no-entry<?php } ?>">
                  <div class="left"><?php _e('City', 'alpha'); ?></div>
                  <div class="right">
                    <?php 
                ...
                <div class="elem w100 <?php if($alert_details['aCategories'] == '' or empty($cat_prepare)) { ?>no-entry<?php } ?>">
                  <div class="left"><?php _e('Categories', 'alpha'); ?></div>
                  <div class="right">
                    <?php 
                      $cats = '';
                ...
                <div class="elem warn"><?php _e('Note that not all conditions are listed, only base alert conditions are shown.', 'alpha'); ?></div>
              </div>

              <div id="alert-items" class="products list">
              <?php while(osc_has_items()) { ?>
                <?php echo alp_draw_item(); ?> 
              <?php } ?>

              <?php if(osc_count_items() == 0) { ?>
                <div class="ua-alert-items-empty ua-alerts"><span><?php _e('No listings match to alert criteria.', 'alpha'); ?></span></div>
              <?php } ?>
              </div>
            </div>
            <?php $c++; ?>
        ...
        <?php } else { ?>
          <div class="ua-items-empty"><img src="<?php echo osc_current_web_theme_url('images/ua-empty.jpg'); ?>"/> <span><?php _e('You have no active alerts', 'alpha'); ?></span></div>
        <?php  } ?>
      </div>
    </div>
  </div>
