...
<body id="body-item-post">
  ...
  <?php
    ...
    if(isset($cookie_loc['fk_c_country_code']) && $cookie_loc['fk_c_country_code'] <> '') {
    ...
    if(isset($cookie_loc['fk_i_region_id']) && $cookie_loc['fk_i_region_id'] <> '') {
  ...
  <div class="content add_item">
    <form name="item" action="<?php echo osc_base_url(true);?>" method="post" enctype="multipart/form-data">
      ...
      <h2 class="post-out"><?php _e('Add free listing', 'zara'); ?></h2>
      ...
      <div id="left">
        ...
        <div class="box photos photoshow <?php if(osc_get_preference('image_upload', 'zara_theme') == 1) { echo 'drag_drop'; } else { echo 'not_drag_drop'; } ?>">
        ...
        <?php
          ...
          $user_info = array();
          $user_info['s_contact_name'] = @$def['s_name'];
          $user_info['s_city_area'] = @$def['s_phone_mobile'];
          $user_info['s_contact_email'] = @$def['s_email'];
        ?>
      ...
      <button type="submit" class="btn btn-primary"><?php _e('Publish item', 'zara'); ?></button>
      </fieldset>
    </form>
  </div>
  ...
  <script>
    ...
    function addNewPhotoZara() {
      ...
      if((max!=0 && num_img<max) || max==0) {
        ...
        alert('<?php echo osc_esc_js(__('Sorry, you have reached the maximum number of images per listing', 'zara')); ?>');
      }
    }
  ...
  <script>
    ...
    // DRAW SELECT IF DEFAULT CATEGORY COMES FROM SEARCH
    <?php if($def_cat['fk_i_category_id'] <> '' && is_numeric($def_cat['fk_i_category_id']) && $def_cat['fk_i_category_id'] > 0) { ?>
      $(document).ready(function(){
