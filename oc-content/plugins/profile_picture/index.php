...
function profile_picture_upload(){ 
  ...
  $height='';
    ...
    echo '<div class="middle avatar-new">';
    echo '<div class="ins">';
  ...
  echo '</div>';
	...
	if($result>0 && file_exists($upload_path.'profile'.$user_id.$result['pic_ext'])){
	    echo '<div>'; // hides form if user already has a profile picture and displays a link to form instead
	}
                ...
                <div class="attachment att-box">
                  <div class="att-wrap">
                    <label class="att-label">
                      <span class="att-btn">' . __('Choose a file', 'profile_picture') . '</span>
                      <span class="att-text">' . __('No file selected', 'profile_picture') . '</span>
                      <input type="file" name="userfile" id="file">
                    </label>
                  </div>
                </div>

	    <input name="Submit" type="submit" id="" class="btn btn-primary alpBg" value="'.$button_text.'">
	    </form>
	    <form name="deleteForm" method="POST" action="'.$url.'"><input type="hidden" name="deletePhoto"></form>
	'; //echo
    	if($result>0 && file_exists($upload_path.'profile'.$user_id.$result['pic_ext'])) { 
          echo '</div>';
	  echo '<div class="links"><a class="rem" href="javascript:deletePhoto();">' . __('Delete picture', 'profile_picture') . '</a></div>';
    ...
    echo '<div class="text">' . __('Upload picture with maximum width of 128px and maximum height of 128px', 'profile_picture') . '</div>';
    echo '</div>';
    echo '</div>';
    echo '</div>';
    echo '</div>';
