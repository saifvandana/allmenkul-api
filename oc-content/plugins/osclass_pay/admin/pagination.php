<?php
function osp_add_pagination($total, $list, $file) {
  ...
  if ($page_count > 1) {
    if (($page + 1) > 1) {
      ...
      // Previous Page			
      if($previous_page > 1) {
        $multiple_pages_p .= ' <a href="' . osc_admin_base_url(true) . '?page=plugins&action=renderplugin&file=' . $file . '&start='.$previous_page.'">&lt;</a> ';
      } else {
        $multiple_pages_p .= ' <a href="' . osc_admin_base_url(true) . '?page=plugins&action=renderplugin&file=' . $file . '">&lt;</a> ';
      }				
    }
    ...
    for ($page_number = 1; $page_number <= $page_count; $page_number++) {
      ...
      if (($page_number - 1) == $page) {
        ...
        if ($page_number <= 11 && $page_number >= $page - 3 || $page_number >= $page - $minus_pages && $page_number <= $page + 5 || $page_number == 1 || $page_number == $page_count) {
          // Other Pages			
          $multiple_pages_links .= ' <a href="' . osc_admin_base_url(true) . '?page=plugins&action=renderplugin&file=' . $file . '&start='.$start_page.'">'. $page_number .'</a> ';
        } else {				
    ...
    if (($page + 1) < $page_count) {
      ...
      // Next Page
      $multiple_pages_n .= ' <a href="' . osc_admin_base_url(true) . '?page=plugins&action=renderplugin&file=' . $file . '&start='.$next_page.'">&gt;</a> ';
    }
  }
