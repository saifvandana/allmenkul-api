<?php
  ...
  $response = StripePayment::processPayment();
  $status = $response[0];
  $message = @$response[1];

  $tx = Params::getParam('stripe_transaction_id');
  ...
  if(OSP_DEBUG) {
    ...
    mail(osc_contact_email() , 'OSCLASS PAY - STRIPE RETURN DEBUG RESPONSE', $emailtext);
  }
