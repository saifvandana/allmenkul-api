<?php
  ...
  $response = CardinityPayment::processPayment();
  $status = $response[0];
  $message = @$response[1];

  $tx = Params::getParam('cardinity_transaction_id');
  ...
  if(OSP_DEBUG) {
    ...
    mail(osc_contact_email() , 'OSCLASS PAY - WEACCEPT RETURN DEBUG RESPONSE', $emailtext);
  }
