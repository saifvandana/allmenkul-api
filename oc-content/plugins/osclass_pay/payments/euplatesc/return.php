<?php
  ...
  $response = EuPlatescPayment::processPayment('RETURN');
  $status = $response[0];
  $message = @$response[1];
  ...
  } else if ($status == OSP_STATUS_FAILED) {
  ...
  if(OSP_DEBUG) {
    ...
    mail(osc_contact_email() , 'OSCLASS PAY - EUPLATESC RETURN DEBUG RESPONSE', $emailtext);
  }
