<?php
  ...
  $response = TwoCheckoutInlinePayment::processPayment();
  $status = $response[0];
  $message = @$response[1];

  $tx = Params::getParam('twocheckout_transaction_id');
  ...
  if(OSP_DEBUG) {
    ...
    mail(osc_contact_email() , 'OSCLASS PAY - 2CHECKOUT RETURN DEBUG RESPONSE', $emailtext);
  }
