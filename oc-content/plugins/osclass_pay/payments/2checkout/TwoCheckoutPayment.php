...
class TwoCheckoutPayment {
  ...
  public static function button($amount = '0.00', $description = '', $itemnumber = '', $extra_array = null) {
    ...
    echo '<li class="payment twocheckout-btn"><a class="osp-has-tooltip" title="' . osc_esc_html(__('Form to enter credit card details will pop-up', 'osclass_pay')) . '" href="#" onclick="twocheckout_pay(\''.$amount.'\',\''.$description.'\',\''.$itemnumber.'\',\''.$extra.'\');return false;" >';
    echo '<span><img src="' . osp_url() . 'img/payments/2checkout.png" alt="' . osc_esc_html(__('2Checkout Payment', 'osclass_pay')) . '"/></span>';
    echo '<strong>' . __('Pay with 2Checkout', 'osclass_pay') . '</strong>';
    echo '</a></li>';
  }
    ...
    try {
      $charge = Twocheckout_Charge::auth(array(
        ...
        "billingAddr" => array(
          ...
          "phoneNumber" => Params::getParam('user_phone')
        )
      ));
      ...
      if ($charge['response']['responseCode'] == 'APPROVED') {
        ...
        if(!$payment) { 
          ...
          // Pay it!
          $payment_details = osp_prepare_payment_data($charge['response']['total'], $payment_id, $data['user'], $product_type);   //amount, payment_id, user_id, product_type
