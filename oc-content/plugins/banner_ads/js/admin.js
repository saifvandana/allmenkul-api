$(document).ready(function(){
  $('body').on('click', 'a.mb-show-banners', function(e) {
    e.preventDefault();
    
    $(this).closest('.mb-table-row').find('.mb-banner-list').slideToggle(200);
  });
  
  // CHECK IF SIZE INPUTS CONTAINS PX OR %
  $('body').on('change keyup keypress', 'input[name="size_width"], input[name="size_height"]', function(e) {
    var val = $(this).val().toLowerCase();
    
    if(val.endsWith("px") || val.endsWith("%")) {
      $('.mb-size-error').hide(0);
      $(this).removeClass('mb-input-error');
    } else {
      $('.mb-size-error').show(0);
      $(this).addClass('mb-input-error');
    }
  });
