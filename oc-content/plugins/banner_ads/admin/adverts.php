<?php
  // Create menu
  $title = __('Adverts', 'banner_ads');
  ba_menu($title);



  $adverts = ModelBA::newInstance()->getAdverts($with_banners = true);
...
<div class="mb-body">
  <!-- CONFIGURE SECTION -->
  <div class="mb-box">
    <div class="mb-head"><i class="fa fa-bookmark-o"></i> <?php _e('Adverts', 'banner_ads'); ?></div>
    ...
    <div class="mb-inside">
      ...
      <div class="mb-table mb-table-adverts">
        <div class="mb-table-head">
          <div class="mb-col-1"><?php _e('ID', 'banner_ads'); ?></div>
          <div class="mb-col-4 mb-align-left"><?php _e('Name', 'banner_ads'); ?></div>
          ...
          <div class="mb-col-4 mb-align-left"><?php _e('Assigned to Banners', 'banner_ads'); ?></div>
          <div class="mb-col-3">&nbsp;</div>
        </div>


        <?php if(count($adverts) <= 0) { ?>
          <div class="mb-table-row mb-row-empty"><i class="fa fa-warning"></i><span><?php _e('No adverts found.', 'banner_ads'); ?></span></div>
        <?php } else { ?>
          <?php foreach($adverts as $a) { ?>
            <div class="mb-table-row">
              ...
              <div class="mb-col-4 mb-align-left">
                <?php if(!isset($a['banners']) || !is_array($a['banners']) || count($a['banners']) <= 0) { ?>
                  <?php $messg = __('This advert has not been assigned/linked to any banner, therefore it will not be visible in front-office. First, add advert into at least one banner.', 'banner_ads'); ?>
                  <em class="mb-blank mb-add-tooltip" title="<?php echo osc_esc_html($messg); ?>"><i class="fa fa-info-circle"></i> <?php _e('No banners assigned (hidden)', 'banner_ads'); ?></em>
                <?php } else { ?>
                  <a href="#" class="mb-show-banners"><?php echo sprintf(__('Assigned to %d banners', 'banner_ads'), count($a['banners'])); ?></a>
                <?php } ?>                   
              ...
              <?php if(isset($a['banners']) && is_array($a['banners']) && count($a['banners']) > 0) { ?>
                <div class="mb-row mb-banner-list">
                  <strong class="mb-titl"><?php _e('List of banners those contains this advert', 'banner_ads'); ?></strong>
                  
                  <div class="mb-table-head">
                    <div class="mb-col-1"><?php _e('ID', 'banner_ads'); ?></div>
                    <div class="mb-col-5 mb-align-left"><?php _e('Banner name', 'banner_ads'); ?></div>
                    <div class="mb-col-4 mb-align-left"><?php _e('Type', 'banner_ads');?></div>
                    <div class="mb-col-4 mb-align-left"><?php _e('Adverts', 'banner_ads'); ?></div>
                    <div class="mb-col-6 mb-align-left"><?php _e('Hooks', 'banner_ads'); ?></div>
                    <div class="mb-col-4">&nbsp;</div>
                  </div>
        
                  <?php foreach($a['banners'] as $b) { ?>
                    <div class="mb-table-row">
                      ...
                      <div class="mb-col-4 mb-align-left">
                        <?php echo sprintf(__('Contains %d adverts', 'banner_ads'), $b['advert_count']); ?>
                      </div>

                      <div class="mb-col-6 mb-align-left mb-hooks"><?php echo (trim($b['s_hook']) <> '' ? $b['s_hook'] : '-'); ?></div>
      ...
      <a href="<?php echo osc_admin_base_url(true); ?>?page=plugins&action=renderplugin&file=banner_ads/admin/advert_edit.php" class="mb-button-green mb-add"><i class="fa fa-plus-circle"></i><?php _e('Create a new advert', 'banner_ads'); ?></a>
